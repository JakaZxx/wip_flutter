...
<div class="container-custom">
    <h1>Buat Pengajuan Peminjaman (officers)</h1>

    <!-- Filter -->
    <form id="filter-form" method="GET" action="{{ route('borrowing.request.create.officers') }}" class="filter-container fade-up" onsubmit="return preserveCartOnFilter(event)">
    ...
    <!-- Form Utama -->
    <form id="borrowing-form" action="{{ route('borrowing.request.store.officers') }}" method="POST">
        @csrf
        ...
        <div id="checkout-modal" class="modal">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <hr>
                    <div class="form-group">
                        <input type="checkbox" id="borrow_for_self" name="borrow_for_self" value="1">
                        <label for="borrow_for_self" class="form-label">Pinjam atas nama sendiri</label>
                    </div>
                    <div class="form-group" id="student_selection_group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="time" id="return_time" name="return_time" class="form-control" value="{{ old('return_time') }}">
                    </div>
                    <div class="form-group">
...
<script>
    ...
    function openModal() {
        ...
        if (hasItems) {
            // Set borrow_date to today's date automatically
            const today = new Date().toISOString().split('T')[0];
            document.getElementById('borrow_date').value = today;
            document.getElementById('borrow_date').readOnly = true;

            // Update return_date min to borrow_date (no +1 day restriction)
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Set current date for borrow_date input
        ...
        // Set current time for borrow_time input
        const hours = String(today.getHours()).padStart(2, '0');
        const minutes = String(today.getMinutes()).padStart(2, '0');
        document.getElementById('borrow_time').value = `${hours}:${minutes}`;

        // Handle borrow_for_self checkbox
        const borrowForSelfCheckbox = document.getElementById('borrow_for_self');
        const studentSelectionGroup = document.getElementById('student_selection_group');
        ...
        borrowForSelfCheckbox.addEventListener('change', function() {
            if (this.checked) {
                ...
                studentIdSelect.value = ''; // Clear selected student
            } else {
        ...
        // Initial state (student selection visible by default)
        studentSelectionGroup.style.display = 'block';
        studentIdSelect.setAttribute('required', 'required');
    });
    ...
    // No additional date validation needed as return_date min is set to borrow_date
