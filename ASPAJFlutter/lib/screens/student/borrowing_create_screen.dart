...
class _BorrowingCreateScreenState extends State<BorrowingCreateScreen> {
  ...
  void initState() {
    super.initState();
    print(
      'BorrowingCreateScreen.initState: Initializing borrowing create screen',
    );
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  ...borrowingProvider.cartItems.map(
                    (item) => CartItemCard(
                      item: item,
                      onRemove: () =>
                          borrowingProvider.removeFromCart(item.commodityId),
                      onUpdateQuantity: (quantity) => borrowingProvider
                          .updateCartItemQuantity(item.commodityId, quantity),
                    ),
                  ),
  ...
  Future<void> _submitBorrowing(BuildContext context) async {
    ...
    final borrowingData = {
      ...
      'status': 'pending',
      'items': borrowingProvider.cartItems
          .map(
            (item) => {
